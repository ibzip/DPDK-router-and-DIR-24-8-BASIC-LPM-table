...
TEST(VERY_SIMPLE_TEST, SIMPLE_ADDRESSES) {
	...
	add_route(IPv4(192,168,20,16), 28, &port_id_to_mac[2], 2); 
	add_route(IPv4(192,168,0,0), 16, &port_id_to_mac[4], 4);
	add_route(IPv4(192,170,10,0), 23, &port_id_to_mac[3], 3);
	add_route(IPv4(192,170,10,10), 27, &port_id_to_mac[4], 4);
	...
	check_address(10, 0, 40, 10, 1);
        check_address(192,168,20,19,2);
	for (int i=0;i<32;i++) {
	  check_address(192,170,10,i,4);
	}
	for (int i=32;i<256;i++) {
	  check_address(192,170,10,i,3);
	}
